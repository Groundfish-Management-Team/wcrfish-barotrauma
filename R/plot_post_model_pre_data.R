...
plot_post_model_pre_data <- function(
  dir, 
  data_list, 
  line_names, 
  ...
  # post-model pre-data
  eta <- rgamma(n, shape = gamma_prior[1], rate = gamma_prior[2])
  mu <- rbeta(n, 1, 1) # uniform
  alpha <- mu * eta
  beta <- (1 - mu) * eta
  beta.var  <- mu * (1 - mu) / (eta + 1)
  beta.sd <- sqrt(beta.var)
  mupred <- rbeta(n, alpha, beta) #p_s
  hist(mupred)
  ...
  hist(mu, col = 'grey', border = 'grey', breaks = seq(0, 1, 0.01), freq = FALSE, axes = FALSE,
